...
public class XWRefreshStateHeader: XWRefreshHeader {
    ...
    */
    override var lastUpdatedateKey:String{
        didSet {
            if let _ = NSUserDefaults.standardUserDefaults().objectForKey(lastUpdatedateKey) {
                
                let formatter: NSDateFormatter = NSDateFormatter()
                formatter.dateFormat = "MMM d, h:mm a"
                let time: String = "Last update: \(formatter.stringFromDate(NSDate()))"
                self.lastUpdatedTimeLabel.text = time
                
            } else {
                self.lastUpdatedTimeLabel.text = "Last updated: No record"
